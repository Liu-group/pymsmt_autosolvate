...
def run_mcpb_orca(inputfile,step):
    addred = 0
    addbpairs = []
    addres = []
    anglefc_avg = 0
    ...
    watermodel = 'opc'
    xstru = 0
    if step not in [ '2', '2s','3','3b', '4', '4b']:
        print('Erorr: invalid options or MCPB orca QM choice!' )
        sys.exit()
    ...
    for line in inputf:
        ...
        #software_version
            ...
            if len(line) == 2:
                g0x = line[1].lower()
                if g0x not in ['g03', 'g09', 'g16', 'gau', 'gms','orca','terachem']:
                    raise pymsmtError('Please use either orca, g03, g09, g16, gau, or '
        ...
        #xstru
            ...
            if len(line) == 2:
                ...
                raise pymsmtError('More than one xstru parameter provided, '
                                'need one.')
    inputf.close()
    print("The input file you are using is : %s" %inputfile)
    print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
...
#Get the renamed residue name
    ...
    if step in ['2', '2s']:
        ...
        if (step in ['2', '2s']): 
            print('start to !!!!!!!!!!!!!!!!!!!!!!!!!!!!')
            if g0x in ['orca','ORCA','Orca']:  ######!!!add orca
                fclogf = gname + '_small_fc.orcaout'
...
if __name__ == "__main__":
    ...
    run_mcpb_orca(inputfile=argumentList[0],step=argumentList[1])